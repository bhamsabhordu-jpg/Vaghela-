    ...
    for symbol in SYMBOLS:
        for tf in TIMEFRAMES:
            try:
                ...
                lines.append(f"{symbol} {tf}: error")
    await tg.send_message(chat_id=CHAT_ID,text="\n".join(lines))

async def main():
    if not TOKEN or not CHAT_ID: raise RuntimeError("Set TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID")
    excls=getattr(ccxt,EXCHANGE_ID)
